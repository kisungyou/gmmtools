...
double eval_gaussian_single(arma::rowvec x, arma::rowvec mu, arma::mat sig, bool logreturn){
  ...
  double add1 = -(dd/2.0)*std::log(2.0*arma::datum::pi);
  double add2 = -0.5*arma::log_det_sympd(sig);
...
double gauss2dist_hd(arma::rowvec m1, arma::mat s1, arma::rowvec m2, arma::mat s2, double theta){
  int d = m1.n_elem;
  double dd = static_cast<double>(d);
  ...
  double term1 = eval_gaussian_single(m1,m1,(2.0*s1 + t2I), false)*thetad*adjpi;
  double term2 = eval_gaussian_single(m2,m2,(2.0*s2 + t2I), false)*thetad*adjpi;
  double term3 = eval_gaussian_single(m1,m2,(s1+s2+t2I), false)*thetad*adjpi;
