...
#' by iteratively merging two nearest components with respect to KL divergence. 
#' Merge is done via moment-preserving merge.
#' 
...
#' gr1 = gmr1990S(gcenter, M=1)
#' gr3 = gmr1990S(gcenter, M=3)
#' gr9 = gmr1990S(gcenter, M=9)
#' 
#' \dontrun{
...
#' 
#' @references 
#' \insertRef{salmond_mixture_1990}{gmmtools}
#' 
#' @concept reduction
#' @export
gmr1990S <- function(gmm, M=2){
  ## INPUTS
  check_gmmobj(gmm, "gmr1990S")
  myk      = max(1, round(M))
  if (myk >= length(as.vector(gmm$weight))){
    k.given = length(as.vector(gmm$weight))
    stop(paste0("* gmr1990S : input 'M' should be smaller than ",k.given," with the given object.."))
  }
  ...
  if (myk < 2){ # case 1 : single component : collapse via MMM
    ...
    cpprun = cpp_reduction_1990S(gmm$weight, gmm$mean, gmm$variance, myk)
