#' GMM Reduction by Runnalls (2007)
#' 
...
#' by iteratively merging two nearest components with respect to KL-derived 
...
#' 
#' @references 
#' \insertRef{runnalls_kullback-leibler_2007}{gmmtools}
#' 
#' @concept reduction
#' @export
gmr2007R <- function(gmm, M=2){
  ## INPUTS
  check_gmmobj(gmm, "gmr2007R")
  myk      = max(1, round(M))
  if (myk >= length(as.vector(gmm$weight))){
    k.given = length(as.vector(gmm$weight))
    stop(paste0("* gmr2007R : input 'M' should be smaller than ",k.given," with the given object.."))
  }
  ...
  if (myk < 2){ # case 1 : single component : collapse via MMM
    ...
    cpprun = cpp_reduction_2007R(gmm$weight, gmm$mean, gmm$variance, myk)
